...
// get the saved notes and populate them in the Article db
...
// delete a specific note in the modal
app.get("/delete/notes/:id", function(req, res){
    db.Note.remove({_id: req.params.id}).then(function(response){
        res.json(response);
    })
});
...
app.get("/delete/saved/:id", function(req, res){
    db.Article.remove({_id: req.params.id}, function(err, updatedDB){
